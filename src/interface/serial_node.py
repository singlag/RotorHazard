        ...
        while success is False and retry_count <= max_retries:
            if check_multi_flag:
                if self.multi_node_index >= 0:
                    self.check_set_multi_node_index(interface);
            try:
    ...
    def write_block(self, interface, command, data, check_multi_flag=True):
        ...
        while success is False and retry_count <= MAX_RETRY_COUNT:
            ...
            try:
                ...
                time.sleep(0.025)  # don't use 'gevent.sleep()' because we don't want other threads jumping in
        return success

    def check_set_multi_node_index(self, interface):
        if self.multi_node_index != self.multi_curnode_index_holder[0]:
            success = False    # need to set different node index on multi-node processor
            retry_count = 0
            out_value = None
            while success is False and retry_count <= MAX_RETRY_COUNT*5:
                ...
                if out_value == self.multi_node_index:
                    ...
                    retry_count = retry_count + 1
                    self.node_log(interface, 'Error setting WRITE_CURNODE_INDEX ({0}): {1}/{2}'.format(retry_count, self.multi_node_index, self.index))
                    self.inc_read_error_count(interface)
                    time.sleep(0.025)  # don't use 'gevent.sleep()' because we don't want other threads jumping in
            if success:
